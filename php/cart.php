...
<head>
  ...
  <title>트리솝에 오신걸 환영합니다.</title>
    <link rel="stylesheet" type="text/css" href="/treesoap/css/cart.css">
      ...
      <div class="main_content">
        ...
        <center>
          ...
          <table style="width:70vw;">
            <tr>
              <th style="border-left:white;" bgcolor="#f0ffe5"><input type="checkbox" id="all_chk" onclick="allchk()" /></th>
              <th bgcolor="#f0ffe5" style="width:120px;">이미지</th>
              <th bgcolor="#f0ffe5" style="width:150px;">상품명</th>
              <th bgcolor="#f0ffe5" style="width:70px;">금액</th>
              <th bgcolor="#f0ffe5" style="width:120px;">적립금</th>
              <th bgcolor="#f0ffe5" style="width:72px;">수량</th>
              <th bgcolor="#f0ffe5">합계</th>
              <th style='border-right:white;' bgcolor="#f0ffe5">선택</th>
            </tr>
        ...
        $que="SELECT *from cart WHERE custom_id='$custom_id'";
        ...
        while($row2=mysqli_fetch_array($res)){
          ...
          echo "<tr>";
            ...
            $que2="SELECT *from product WHERE prd_id={$row2['prd_id']}";
            $res2=mysqli_query($conn,$que2);
            echo mysqli_error($conn);
            while($row3=mysqli_fetch_array($res2)){
              echo"<tr>";
              echo "<th style='border-left:white;' border-top:'1px solid #dbdbdb;'><input name='check[]' id='chk' type=checkbox value=".$row2['wdate']." ></th>";
                if($row3['prd_photo']){ $prd_photo=$row3['prd_photo']; echo "<td> <img class='products_cart'  src='".$prd_photo."' /></td>"; }

                if($row3['prd_name']){$prd_name=$row3['prd_name']; echo "<td>".$prd_name."</td>"; }
                if($row3['prd_price']){$prd_price=$row3['prd_price']; echo"<td>".$prd_price."</td>";}
                if($row3['prd_point']){$prd_point=$row3['prd_point']; echo"<td>".$prd_point."</td>";}
            }

            if($row2['prd_num']){$prd_num=$row2['prd_num'];echo "<td>".$prd_num."</td>";}


            if($row2['total_price']){$total_price=$row2['total_price']; echo "<td style='border-right:white;'>".$total_price."</td>";}
             echo "<form  method='POST' action='/treesoap/cart_delete.php'><td style='border-right:white; text-align:center;'> <button class='deletebtn' formaction='/treesoap/cart_delete.php'/> <a>삭제</a></button>
             </td></tr>
        ...
        <div style="float:left; margin-top:5vw; margin-bottom:5vw; margin-left:15vw; ">
          <form method='POST' action='/treesoap/cart_deleteall.php'>
            <td style='border-right:white; text-align:center;'> <button class="paybtn1">장바구니 비우기 </button>
            </td>
            </tr>
        ...
        <div style="float:right; margin-top:5vw; margin-bottom:5vw; margin-right:15vw;">
          <button class="paybtn1" onclick="location.href='/treesoap/payment.php';">선택 상품 주문 </button>
          <button class="paybtn2" onclick="location.href='/treesoap/payment.php'; checkAll(true);">전체 상품 주문 </button>
        </div>
