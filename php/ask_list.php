...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="/treesoap/css/ask_list.css">
...
<body>
  ...
  <?php
            ...
            if($num!==""){
              $que="SELECT * from board_ask  WHERE num='$num'";
              ...
              if($row=mysqli_fetch_array($result)){
                ...
                if($row['ask_content']){
                $ask_content=$row['ask_content'];}
                if($row['custom_id']){
                $custom_id=$row['custom_id'];}
                if($row['ask_photo'])$ask_photo=$row['ask_photo'];


              }
              $que2="SELECT * from member  WHERE custom_id='$custom_id'";
              $result2=mysqli_query($conn,$que2);

              if($row2=mysqli_fetch_array($result2)){
                if($row2['userid'])$userid=$row2['userid'];
              }
  ...
  <div class="main_box">
    <div class="section">
      <div class="section-header">
        <h2 class="h2" style="    font-size: 28px;    color: #4e4e4e;">문의하기</h2>
      ...
      <div class="section-body">
        <div class="board-view">
          <div class="board-view-head">
            ...
            <div class="board-view-info">
              <div class="author">
                <span class="text1"><strong><?php echo "$userid" ?></strong>
                </span>
                <span class="divide-bar">&nbsp;</span>
                <span class="text2"><?php echo "$ask_date"; ?></span>
              </div>
              <div class="value">
                <span class="text4">
                  <!--<span class="divide-bar"></span>-->
          ...
          <div class="board-view-body">
            <!--상품.주문연동-->
            <div class="itemorder-wrap select-goods" style="border-bottom:gray dashed 1px;">
              <div class="itemorder-head">
                <a>
                  <img src="<?= $ask_photo?>">
                </a>
              </div>
              <div class="textfield" style="float:left;">
                <?php echo "$ask_content";?>
              </div>
            </div>
        ...
        <div class="ta-r m2 board-ctrl-btn">

          <form action="/treesoap/ask_delete.php" method="post">
            <a class="skinbtn" onclick="history.back(1);"><span>목록</span></a>
            <button style="background-color: white; float:right;" class="skinbtn"><span>삭제</span></button>
            <input type="hidden" name="deleteid" value="<?=$row['custom_id']?>">
            <input type="hidden" name="deletedate" value="<?=$ask_date?>">
          </form>
        </div>
      </div>
    </div>
